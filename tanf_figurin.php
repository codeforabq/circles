...
if(isset($_POST['cl_child8_years'])) {
	if($_POST['cl_child8_years'] >= 2) {
		$children_over2 = $children_over2 + 1;
	} else {
		$children_under2 = $children_under2 + 1;
	}
...
if(isset($_POST['cl_child11_years'])) {
	if($_POST['cl_child11_years'] >= 2) {
		$children_over2 = $children_over2 + 1;
	} else {
		$children_under2 = $children_under2 + 1;
	}
...
while($incadj_row = $incadj_result->fetch_assoc()) {
	if(($cl_adults_PT > 0) || ($cl_adults_FT > 0)) {
		$first_adult_deduct = $incadj_row['income_adjust_firstadult'];
	} else {
		$first_adult_deduct = "0";
	}
	if($cl_adults_PT + $cl_adults_FT > 1) {
		$second_adult_deduct = $incadj_row['income_adjust_secondadult'];
	} else {
		$second_adult_deduct = "0";
	}
}
...
while($deductcs_row = $deductcs_result->fetch_assoc()) {
	...
	if($deduct_childsupport_paid == "FALSE") {
		$deduct_childsupport_amount = "0";
	}
}
...
while($deductep_row = $deductep_result->fetch_assoc()) {
	...
	if($deduct_expensespaid_forincome == "FALSE") {
		$deduct_incomeexpenses_amount = "0";
	} else {
		$deduct_incomeexpenses_amount = $incomeexpenses_amount;
	}
}
...
while($deductcsr_row = $deductcsr_result->fetch_assoc()) {
	if($cl_less_than_thirteen + $cl_thirteen_to_twenty == 1) {
		...
		$child_support_received1_amount = 0;
	}
	...
	if($cl_less_than_thirteen + $cl_thirteen_to_twenty > 1) {
		...
		$child_support_received2_amount = 0;
	}
}
...
while($deductelddis_row = $deductelddis_result->fetch_assoc()) {
	if($deductelddis_row['medical_elderly_disabled_allowed'] == "TRUE") {
		...
		$medical_elderly_disabled_amount = 0;
	}
}
...
if($TotalIncome <= $gross_income_limit_amount) {
	$gross_income_test = "TRUE";
} else {
	$gross_income_test = "FALSE";
}

if($cl_less_than_thirteen + $teenager_count > 0) {
	$child_test = "TRUE";
} else {
	$child_test = "FALSE";
}
...
if($people_count <= 8) {
	$family_size_lookup = $people_count;
} else {
	$family_size_lookup = 8;
}
if($people_count > 8) {
	$add_family_lookup = $people_count - 8;
} else {
	$add_family_lookup = 0;
}
$standard_of_need_total = $fpig100 + $additional_family_amount;
if($CountableIncome < $standard_of_need_total) {
	$standard_of_need_test = "TRUE";
} else {
	$standard_of_need_test = "FALSE";
}
...
while($eligibleresources_row = $eligibleresources_result->fetch_assoc()) {
	if($eligibleresources_row['vehicle_equity'] = 1) {
		...
		if(($Resource_Limit_Amount > $Adjusted_Value_of_Assets) || ($Resource_Limit_Amount < 0)) {
			$Resource_Test_Passed = "TRUE";
		} else {
			$Resource_Limit_Amount = "FALSE";
		}
	} else {
		$Resource_Test_Passed = "TRUE";
	}
