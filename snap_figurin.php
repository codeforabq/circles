...
//THIS VALUE ISN'T IN ANY TABLE . . . HARDCODED!
      ...
      $SNAP_Max_People_Count=10;
          ...
          if (isset($_POST['cl_family_over60_check'])){$elderly_disabled_home=$_POST['cl_family_over60_check'];}
...
//LOOKUP RESOURCE LIMITS
      ...
      if ($vehicle_equity_allowed=="0"){$EVvehicle1="0"; $EVothervehicle="0";}
      ...
      if ($resource_limit_amount>$adjusted_asset_value){$passSNAP_resource_test="True";}else{$passSNAP_resource_test="False";}

      if ($elderly_disabled_home=="No"){$household_elderly_code=0;}else{$household_elderly_code=1;}
      //if ($homeless_home=="No"){$homelesscode=0;}else{$homelesscode=2;}
      $homelesscode=0;
      if ($every_member_benefits=="No"){$everymembercode=0;}else{$everymembercode=4;}
      $household_status_code=$household_elderly_code+$homelesscode+$everymembercode;

      if ($TotalIncome<$Gross_Income_Limit_AmountSNAP){$gross_income_testSNAP="TRUE";}else{$gross_income_testSNAP="FALSE";}

      if ($household_status_code>3)
        {
          $shall_we_proceed="Yes";
          if ($TotalIncome>$Gross_Income_Limit_AmountSNAP){$shall_we_proceed="No";}else{$shall_we_proceed="Yes";}
        }

//DETERMINE NET INCOME ELIGIBILITY
    if (isset($_POST['UnEmploymentEtc'])){$unearned_income_received=$_POST['UnEmploymentEtc'];}else{$unearned_income_received=0;}
    if (isset($_POST['MedicalElderlyAmount'])){$MedicalElderlyAmount=$_POST['MedicalElderlyAmount'];}else{$MedicalElderlyAmount=0;}
...
//LOOKUP SNAP DISREGARD PERCENTAGE
      ...
      $stddeduct_result = $dbconn->query($stddeduct_sql);
        ...
        if ($elderly_disabled_home=="Yes")
        {
          ...
          else{$medical_expense_allowanceSNAP=0;}
...
//CALCULATE EXCESS SHELTER DEDUCTION
        ...
        if($household_status_code>$SNAP_Net_Food_Assistance_Income)
          ...
          else{$SNAP_Net_Income_Test="False";}
