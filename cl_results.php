...
<div id="MainDIV" class="centered">
			...
			Children -
				...
				<?php
						...
						$TotalIncome = $TotalEarnedIncome+$TotalUnEarnedIncome;
								...
								if($people_count>8){$hud_people_count=8;}else{$hud_people_count=$people_count;}
								...
								if (isset($_POST['Bedrooms'])){$cl_Bedrooms=$_POST['Bedrooms'];}else{$cl_Bedrooms=0;}
								if ($cl_Bedrooms=0){$HUD_Room_Adjusment=0.5;}
								if ($cl_Bedrooms=1){$HUD_Room_Adjusment=0.7;}
								if ($cl_Bedrooms=2){$HUD_Room_Adjusment=0.9;}
								if ($cl_Bedrooms=3){$HUD_Room_Adjusment=1.1;}
								if ($cl_Bedrooms=4){$HUD_Room_Adjusment=1.5;}
								if ($cl_Bedrooms=5){$HUD_Room_Adjusment=1.6;}

								if (isset($_POST['AlimonyReceived'])){$MAGI_AlimonyReceived=$_POST['AlimonyReceived'];}else{$MAGI_AlimonyReceived=0;}
								if (isset($_POST['MonthlyGifts'])){$MAGI_MonthlyGifts=$_POST['MonthlyGifts'];}else{$MAGI_MonthlyGifts=0;}
								if (isset($_POST['ArmedForcesAmount'])){$MAGI_ArmedForcesAmount=$_POST['ArmedForcesAmount'];}else{$MAGI_ArmedForcesAmount=0;}
								if (isset($_POST['cl_family_college_students'])){$Student_Count=$_POST['cl_family_college_students'];}
									else{$Student_Count=0;}
								if (isset($_POST['GasAmount'])){$cl_GasAmount=$_POST['GasAmount'];}else{$cl_GasAmount=0;}
								if (isset($_POST['ElectricAmount'])){$cl_ElectricAmount=$_POST['ElectricAmount'];}else{$cl_ElectricAmount=0;}
    ...
    {
      echo "<span style=\"color:red;\" class=\"p2\">A total montly income of $".number_format($TotalIncome,2)." falls outside the data available for <br>subsidy calculations and graphing for a ".$people_count." person home in ".$_POST["cl_city"].", ".$cl_county.", ".$cl_state."!<br><br></span>"; exit;
    }
     ?>
		</div>
		<br>
			...
			while ($x<=$plotpoints)
				{
					if ($x==1)
						{
							if ($current_hourly==0){$current_line=0;}
              $this_hourly=0;
							${"this_hourly".$x}="0.00";
				...
				while ($x<=$plotpoints)
					{
						...
						if($This_TotalIncome<$Gross_Income_Limit_AmountSNAP){$This_SNAP_Gross_Income_Test="True";}else{$This_SNAP_Gross_Income_Test="False";}
						...
						if($fpig100>$SNAP_This_NetFood_Assistance_Income){$SNAP_This_NetIncomeLimit_Test="True";}else{$SNAP_This_NetIncomeLimit_Test="False";}
		          ...
		          else{$SNAP_This_EBT_Lookup_Positive=0;}
			...
			while ($x<=$plotpoints)
				{
					...
					if(${"this_monthly".$x}<=$gross_income_limit_amount){$this_gross_income_test="TRUE";}else{$this_gross_income_test="FALSE";}

					if($this_countable_income<$standard_of_need_total){$this_adjustment_test="TRUE";}else{$this_adjustment_test="FALSE";}
					...
					if (($this_gross_income_test="TRUE")&&($child_test=="TRUE")&&($this_adjustment_test="TRUE")&&($Resource_Test_Passed="TRUE"))
					{
						...
						if ($This_Monthly_Assist_Payment>=$States_Minimum_Payment){$This_TANF_Subsidy_Amount=$This_Monthly_Assist_Payment;}else{$This_TANF_Subsidy_Amount=0;}

						${"this_tanf".$x}=$This_TANF_Subsidy_Amount;
						if ($This_TANF_Subsidy_Amount>0){echo "<td>".number_format($This_TANF_Subsidy_Amount)."</td>";}
						 	else{echo "<td bgcolor=\"lightgray\">".number_format($This_TANF_Subsidy_Amount)."</td>";}
					}
            else{${"this_tanf".$x}=0;echo "<td bgcolor=\"lightgray\">0</td>";}
							...
							if ($HowManyChildren!=0)
								{
									...
									if (($CCIS_KidsLessThan13_Amount+$CCIS_KiddosUnable_Amount)>.5){$CCIS_ChldAge_Test="True";}else{$CCIS_ChldAge_Test="False";}
					...
					while ($x<=$plotpoints)
						{
							...
							if ($MAGI_Adjusted_Gross_Income<=$MAGI_FPIG133){$MAGI_Adjusted_Monthly_Income=$MAGI_Adjusted_Gross_Income;}
								else{$MAGI_Adjusted_Monthly_Income=$MAGI_Adjusted_Gross_Income-$MAGI_5Exclusion;}

							if ($MAGI_Adjusted_Monthly_Income<=$MAGI_FPIG133){$MAGI_Eligible="True";}else{$MAGI_Eligible="False";}
		 ...
		 <tr>
		  <td style="font-weight:bold;">HUD Subsidy Pmt</td>
			<?php
					...
					if($elderly_disabled_home=="Yes"){$HUD_ElderlyDisabled_Allowance=400;}else{$HUD_ElderlyDisabled_Allowance=0;}

					$EID_Count=0;
					if (isset($_POST['Butt_Section8'])){if ($_POST['Butt_Section8']=="Yes"){$EID_Count++;}}
					if (isset($_POST['EIupemp'])){if ($_POST['EIupemp']=="Yes"){$EID_Count++;}}
					if (isset($_POST['EIupPart'])){if ($_POST['EIupPart']=="Yes"){$EID_Count++;}}
					if (isset($_POST['IncomeUp'])){if ($_POST['IncomeUp']=="Yes"){$EID_Count++;}}
					if ($EID_Count>=2){$EID_Qualify="Qualified";}else{$EID_Qualify="Not Qualified";}
					...
					while ($x<=$plotpoints)
						{
									...
									if ($HUD_Income_Limit>$HUD_Annual_Income){$HUD_Income_Test="True";}else{$HUD_Income_Test="False";}
									$HUD_NonReimbursed_Medical=$ExpenseDisabledAmount+$medical_elderly_amount;
									$HUD_3Annual_Income=$HUD_Annual_Income*.03;
									if ($HUD_NonReimbursed_Medical-$HUD_3Annual_Income>0){$HUD_Allowable_MedExpense=$HUD_NonReimbursed_Medical-$HUD_3Annual_Income;}
										else{$HUD_Allowable_MedExpense=0;}
									...
									if ($HUD_Annual_Income-$HUD_EID_Allowances<0){$HUD_Monthly_Adjusted_Income=0;}
									else{$HUD_Monthly_Adjusted_Income=($HUD_Annual_Income-$HUD_EID_Allowances)/12;}

									$HUD_30Monthly_Adjusted_Income=$HUD_Monthly_Adjusted_Income*.3;
									$HUD_10Monthly_Gross_Income=${"this_monthly".$x}*.1;

									if($HUD_30Monthly_Adjusted_Income>$HUD_10Monthly_Gross_Income){$HUD_TenantRent_Amount=$HUD_30Monthly_Adjusted_Income;}
										else{$HUD_TenantRent_Amount=$HUD_10Monthly_Gross_Income;}
									...
									if ($cl_utilities=="sua2")
										{
											...
											if ($HUD_AdjustedRent_Amount<0){$HUD_Rent_Subsidy=$SNAP_MonthlyRentMortgage;}
												else{$HUD_Rent_Subsidy=$SNAP_MonthlyRentMortgage-$HUD_TenantRent_Amount;}
										}
											else
											{
